...
processRaster <- function(raster, targetRaster) {
  source_resolution <- res(raster)[1] # Assuming uniform resolution across dimensions
  target_res <- res(targetRaster)[1] # Assuming uniform resolution for target raster
  factor <- target_res / source_resolution
  
  # Only perform aggregation if factor > 1
  if (factor > 1) {
    aggregation_factor <- round(factor)
    aggregated_raster <- aggregate(raster, fact=aggregation_factor, fun=modeFun)
    cat("Aggregated raster resolution:", res(aggregated_raster), "\n")
    return(aggregated_raster)
  } else {
    cat("No aggregation performed. Source resolution is coarser than or equal to the target resolution.\n")
    return(raster)
  }
}

# Process both rasters to match the resolution and extent of the created empty raster 'r'
res_lc_2013 <- processRaster(lc_2013, r)
res_lc_2023 <- processRaster(lc_2023, r)
