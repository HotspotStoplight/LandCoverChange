...
create_and_upload_plot <- function(plot_code, plot_name, output_folder, outputs) {
  # Ensure the local output directory exists
  full_output_path <- file.path(output_folder, outputs)
  if(!dir.exists(full_output_path)) {
    dir.create(full_output_path, recursive = TRUE)
    cat("Directory created: ", full_output_path, "\n")
  } else {
    cat("Directory already exists: ", full_output_path, "\n")
  }

  # Create a temporary file path for the plot
  temp_file <- tempfile(fileext = ".png")
  cat("Temporary file path: ", temp_file, "\n")
  ...
  if(file.info(temp_file)$size == 0) {
    stop("No data written to the temporary file.")
  }
